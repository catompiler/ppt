...
static void* drive_tasks_write_event_osc_task(void* event)
{
    err_t err = E_NO_ERROR;
...
err_t drive_tasks_write_error_event(void)
{
    ...
    return TID_TO_ERR(scheduler_add_task(drive_tasks_write_event_osc_task,
...
{
    static drive_event_t event;
    drive_events_make_event(&event, DRIVE_EVENT_TYPE_WARNING);
    
    return TID_TO_ERR(scheduler_add_task(drive_tasks_write_event_task,
                DRIVE_TASKS_WRITE_WARNING_EVENT_TASK_PRIOR, &event, TASK_RUN_ONCE, NULL));
}
