...
//! Высокоточный таймер.
static drive_hires_timer_t hires_timer;
...
static void init_timer_priph(TIM_TypeDef* TIM)
{
    ...
    TIM->CR2 = 0;
    TIM->DIER = 0;
    TIM->PSC = DRIVE_HIRES_TIMER_PRESCALER - 1;
    TIM->ARR = DRIVE_HIRES_TIMER_PERIOD_TICKS - 1;
    TIM->CNT = 0;
    TIM->SR  = 0;
}


err_t drive_hires_timer_init(TIM_TypeDef* timer)
{
...
void drive_hires_timer_irq_handler(void)
{
...
bool drive_hires_timer_irq_enabled(void)
{
    return hires_timer.timer->DIER & TIM_DIER_UIE;
}

void drive_hires_timer_irq_set_enabled(bool enabled)
{
    if(enabled){
...
bool drive_hires_timer_running(void)
{
    return hires_timer.timer->CR1 & TIM_CR1_CEN;
}

void drive_hires_timer_set_running(bool running)
{
    if(running){
...
void drive_hires_timer_start(void)
{
    hires_timer.timer->CR1 |= TIM_CR1_CEN;
}

void drive_hires_timer_stop(void)
{
    hires_timer.timer->CR1 &= ~TIM_CR1_CEN;
}

void drive_hires_timer_reset(void)
{
    hires_timer.timer->CNT = 0;
}

void drive_hires_timer_value(struct timeval* tv)
{
    if(tv == NULL) return;
